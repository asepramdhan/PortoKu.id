...
new class extends Component {
    ...
    public function editField($postId, $field)
    {
        ...
        if ($field === "title") {
            ...
            $this->editingValue = $post->published_at
    ...
    public function saveField()
    {
        ...
        if ($this->editingField === "status") {
            ...
            $post->update([
                "published_at" =>
                    $this->editingValue === "published" ? now() : null,
            ]);
        }
    ...
    public function updatedImageUpload()
    {
        ...
        if ($post) {
            ...
            $post->update([
                "featured_image_path" => Storage::disk("public")->url($path),
            ]);
    ...
    public function deletePost()
    {
        if ($this->deleting) {
            ...
            if ($this->deleting->featured_image_path) {
                $path = Str::after(
                    $this->deleting->featured_image_path,
                    "/storage/",
                );
                if (Storage::disk("public")->exists($path)) {
                    Storage::disk("public")->delete($path);
            ...
            session()->flash(
                "message",
                "Postingan dan gambar terkait berhasil dihapus.",
            );
        }
                ...
                <tbody>
                    @forelse ($posts as $post)
                        <tr wire:key="{{ $post->id }}">
                            <td>
                                <label
                                    for="image-upload-{{ $post->id }}"
                                    class="cursor-pointer relative block w-20 h-12"
                                    wire:click="$set('postIdForImageUpload', {{ $post->id }})"
                                >
                            ...
                            <td class="truncate">
                                @if ($editingId === $post->id && $editingField === "title")
                                    <input
                                        ...
                                        x-ref="editInput{{ $post->id }}_title"
                                        @click.away="$wire.cancelEdit()"
                                        x-trap.noscroll
                                    />
                                @else
                                    <p
                                        wire:click="editField({{ $post->id }}, 'title')"
                            ...
                            <td class="whitespace-nowrap">
                                @if ($editingId === $post->id && $editingField === "status")
                                    <div class="flex items-center gap-2">
                                        <select
                                            ...
                                            wire:keydown.escape="cancelEdit"
                                            class="bg-slate-800 text-white text-sm rounded-md border-slate-600 focus:border-sky-500 focus:ring-sky-500"
                                            x-init="$nextTick(() => $el.focus())"
                                            x-ref="editInput{{ $post->id }}_status"
                                            @click.away="$wire.cancelEdit()"
                                            x-trap.noscroll
                                        >
                                ...
                                @else
                                    <div
                                        wire:click="editField({{ $post->id }}, 'status')"
                                        class="cursor-pointer hover:bg-slate-700 p-1 rounded"
                                    >
                                        @if ($post->published_at && ! $post->published_at->isFuture())
                                            <span
                                                class="status-badge bg-green-500/20 text-green-400"
                                            >
                                                Diterbitkan
                                            </span>
                                        @else
                                            <span
                                                class="status-badge bg-yellow-500/20 text-yellow-400"
                                            >
                                                Draf
                                            </span>
                                        @endif
                            ...
                            <td>
                                <div class="flex space-x-4 lg:space-x-2">
                                    ...
                                    <button
                                        wire:click="prepareToDelete({{ $post->id }})"
