    ...
    <div class="w-full max-w-md space-y-8">
      <div>
        ...
        <p class="mt-2 text-center text-sm text-slate-400">
          Atau <a href="/register" wire:navigate class="font-medium text-sky-400 hover:text-sky-500">buat akun baru</a> jika Anda belum terdaftar.
        </p>
      </div>
      ...
      <form class="mt-8 space-y-6" wire:submit.prevent="login">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="text-sm">
            <a href="/forgot-password" wire:navigate class="font-medium text-sky-400 hover:text-sky-500">
              Lupa kata sandi?
            </a>
          </div>
        </div>
        ...
        <div>
          <button type="submit" wire:loading.attr="disabled" wire:target="login" class="group relative w-full flex justify-center py-3 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-sky-600 hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 focus:ring-sky-500 cursor-pointer">
            <div class="flex items-center">
              <x-loading wire:loading wire:target="login" class="loading-dots mr-2" />
              <x-icon name="lucide.log-in" wire:loading.remove wire:target="login" class="mr-2" />
              Login
            </div>
          </button>
        </div>
