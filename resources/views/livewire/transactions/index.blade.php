...
new class extends Component {
  ...
  // Filter properties
  public $filterType = '', $filterAsset = '', $search = '', $filterDate = '';
  ...
  public $edit_type, $edit_asset_id, $edit_quantity, $edit_price_per_unit, $edit_amount, $edit_category, $edit_notes, $edit_transaction_date;
  ...
  public function editField($transactionId, $field)
  {
    ...
    if ($field === 'amount') {
      $this->editingValue = number_format($transaction->amount ?? 0, 0, ',', '.');
      } elseif ($field === 'quantity') {
        // Gunakan number_format dengan . sebagai pemisah desimal agar cocok dengan x-mask
        $this->editingValue = number_format($transaction->quantity, 8, '.', ',');
        ...
        $dateToDisplay = $transaction->transaction_date ?? $transaction->updated_at;
  ...
  public function saveField()
  {
    ...
    } elseif ($this->editingField === 'transaction_date') { // Aturan untuk inline edit tanggal
  ...
  public function prepareToEdit(FinancialEntry $entry): void
  {
    ...
    $this->edit_price_per_unit = number_format($this->edit_price_per_unit ?? 0, 0, ',', '.');
    $this->edit_amount = number_format($this->edit_amount ?? 0, 0, ',', '.');
  ...
  public function updateTransaction(): void
  {
    ...
    $dataToValidate = [
        ...
        'quantity' => ($this->edit_type === 'buy' || $this->edit_type === 'sell') ? (float) $editQuantity : null, 
        'price_per_unit' => ($this->edit_type === 'buy' || $this->edit_type === 'sell') ? (float) $editPricePerUnit : null,
    ];
    ...
    if ($this->filterDate) {
      ...
      $query->where(function($q) {
        $q->whereDate('transaction_date', $this->filterDate)
          ->orWhere(function($sq) {
            $sq->whereNull('transaction_date')
              ->whereDate('updated_at', $this->filterDate);
          });
      });
    }
...
<div>
  ...
  <div class="mb-6 flex flex-col justify-between md:flex-row md:items-center">
    ...
    <button wire:click="prepareToAdd" class="mt-4 flex cursor-pointer items-center gap-2 rounded-lg bg-sky-500 px-4 py-2 font-semibold text-white transition-colors hover:bg-sky-600 md:mt-0">
        ...
        <div class="mt-6 flex justify-center gap-4">
          ...
          <button type="button" wire:click="deleteTransaction" wire:loading.attr="disabled" wire:target="deleteTransaction" class="w-full cursor-pointer rounded-lg bg-red-600 px-6 py-2 font-semibold text-white hover:bg-red-700">
            <div class="flex items-center justify-center">
              <x-loading wire:loading wire:target="deleteTransaction" class="loading-dots mr-2" />
              <x-icon name="lucide.trash-2" wire:loading.remove wire:target="deleteTransaction" class="mr-2" />
              Ya, Hapus
            </div>
          </button>
        </div>
